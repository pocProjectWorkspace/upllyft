              ...
              {clinics.map((clinic) => (
                ...
                >
                  <div className="p-6 relative">
                    {isParent && (
                      <button
                        onClick={() => toggleSave.mutate({ clinicId: clinic.id })}
                        aria-label={savedIds.has(clinic.id) ? 'Remove from saved' : 'Save clinic'}
                        className={`absolute top-4 right-4 p-1.5 rounded-lg transition-colors ${
                          savedIds.has(clinic.id) ? 'text-rose-500' : 'text-gray-300 hover:text-rose-400'
                        }`}
                      >
                        <svg className="w-5 h-5" fill={savedIds.has(clinic.id) ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
