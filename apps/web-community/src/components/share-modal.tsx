  ...
  const handleNativeShare = useCallback(async () => {
    try {
      ...
      if (err instanceof Error && err.name !== 'AbortError') {
        toast({ title: 'Share failed', description: 'Please try another method.', variant: 'destructive' });
      }
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Header */}
        <h2 className="text-lg font-semibold text-gray-900 mb-1">Share Post</h2>
        <p className="text-sm text-gray-500 mb-5 pr-6">
          Share &ldquo;{postTitle.length > 50 ? postTitle.slice(0, 50) + '...' : postTitle}&rdquo; with your network
        </p>
        ...
        {supportsNativeShare && (
          <button
            ...
            className="w-full bg-teal-500 hover:bg-teal-600 text-white font-semibold py-3 rounded-xl flex items-center justify-center gap-2 transition mb-4"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="grid grid-cols-3 gap-3">
          {platforms.map((platform) => (
            <button
              ...
              className={`flex flex-col items-center justify-center gap-2 py-4 px-2 border border-gray-200 rounded-xl transition cursor-pointer ${platform.hoverBorder} ${platform.hoverBg} ${platform.hoverText} text-gray-700`}
            >
              {platform.icon}
        ...
        <div className="mt-5">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-500 truncate"
            />
            <button
