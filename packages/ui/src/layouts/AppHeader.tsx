        ...
        <div className="flex items-center justify-between h-14 md:h-16">
          ...
          <div className="flex items-center gap-2 md:gap-8 min-w-0 flex-1">
            ...
            {/* Desktop navigation — items with sub-destinations open a dropdown;
                the rest are plain links. One row, no second-level strip. */}
            <nav className="hidden md:flex items-center gap-1 min-w-0 overflow-hidden">
              {globalNav.map((item) => {
                const isActive = item.app === currentApp;
                const pillClass = `px-3 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${
                  isActive
                ...
                return (
                  <DropdownMenu key={item.label}>
                    <DropdownMenuTrigger className={`${pillClass} inline-flex items-center gap-1 outline-none`}>
                      {item.label}
            ...
            {globalNav.map((item) => {
              ...
              return (
                <div key={item.label} className="pt-1">
                  <p className={`px-3 pt-2 pb-1 text-xs font-semibold uppercase tracking-wide ${isActive ? 'text-teal-600' : 'text-gray-400'}`}>
                    {item.label}
                  </p>
                  {item.children.map((child) => (
                    <a
